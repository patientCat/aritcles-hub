...
![Reflection flow](https://raw.githubusercontent.com/langchain-ai/langgraphjs/b7c196b2142fb888dfcd9ceb1dfb4365d803c8b6/examples/reflection/img/reflection.png)
...
// 第一轮
messages = [
  HumanMessage("写一篇关于 X 的论文"),   // 用户输入
  AIMessage("初稿..."),                   // 生成器输出
  HumanMessage("反思：结构不够清晰..."),  // 反思结果
]

// 第二轮：生成器收到上轮的 HumanMessage（反思内容）
// 把它理解为"用户在给我批评意见"，于是生成改进版
messages = [
  HumanMessage("写一篇关于 X 的论文"),
  AIMessage("初稿..."),
  HumanMessage("反思：结构不够清晰..."),
  AIMessage("改进稿..."),                 // 生成器基于用户反馈继续生成
  HumanMessage("反思：论据不够充分..."),
]

// 第三轮同理...
```

问题是：**生成器（第一轮）输出的 AIMessage 内容是"我（AI）写的初稿"**。如果直接把这个 AIMessage 传给下一轮，生成器会困惑："这是我说的话，为什么要我再改？"
...
// 原本: AIMessage("初稿...")  AIMessage("反思...")
// 变成: HumanMessage("初稿...") HumanMessage("反思...")
```
...
生成器会理解为：**用户在给我反馈，告诉我初稿哪里不好**。这比"AI 在自言自语"更容易让模型接受并改进。
...
const shouldContinue = (state: typeof State.State) => {
  ...
  if (messages.length > 6) return END;  // 确定性判断，不是 LLM
  return "reflect";
};
...
`shouldContinue` 是一个普通函数，做确定性判断（这里是 `messages.length > 6`），决定下一步跳到 `reflect` 还是 `END`。流程图上边的条件是显式声明的，比在代码里写 `for` 循环更容易看清楚整个流转逻辑。
...
三个地方更新方式都不一样：`push`、`直接赋值`、`??`。这就是**状态更新的语义分散在使用方**，每次更新都要想"这里该 push 还是赋值"。
