...
其实在最一开始的时候， 我只是想着去学习下这个Agent工具如何构建。 以我自己的能力，能否搭建一个Codebuddy， 或者说Codebaby。
在这个过程中，我的思路开是逐渐清晰， 我打算尝试去做一个Codebaby出来。 当涉及到工程化的东西的时候， 就不是需要简单的之前的toy代码可以运行的了。 这类代码在讲解原理的时候， 很容易理解， 但是在很多的边界处理上，非常差。 所以我引入了一个比较基础的Agent框架———openai/agents
...
- Agent 的稳定性，来自工具边界和状态设计
- Agent 的可维护性，来自日志和可验证流程

到这里，你已经不只是“让它回答问题”，而是在构建一个可演进的 Agent 系统。
